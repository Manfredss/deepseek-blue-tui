...
- **流式回复不再等到整行才显示**。0.3.0 引入的 Markdown 渲染逐行缓冲——必须看到换行才能判定这行是不是代码围栏/标题/列表——导致没有换行的段落在生成期间屏幕上毫无反应，写完才一次性蹦出来。中文自然段常一两百字不换行，按 50 tok/s 就是好几秒死寂，比渲染之前的手感更差
  - 散文与列表项现在边到边输出；只有必须整行判定的（围栏、标题、引用、表格、分隔线）仍然缓冲
  - 行内标记未闭合时只扣住它自己那一小段，闭合后连同样式一起补出
...
- **流式 Markdown 渲染**：此前助手回复是原样吐进终端的，代码块没有任何处理——对一个主要用来问编程问题的客户端，这是最大的体验缺口
  - 围栏代码块带语言标签、左侧竖线与轻量着色（注释、字符串、数字、关键字）；着色器逐字符从左到右扫描，因此字符串里的 `//` 不会被误判为注释
  - 代码**绝不截断或重排**：丢一个字符比换行难看严重得多，超长行交给终端自己折
  - 标题、列表、引用、分隔线、行内 `` `代码` ``/**粗体**/链接均按结构渲染
  - 逐行缓冲，所以无论 SSE 分片切在哪里（哪怕切在围栏中间）结果都一致；流中断时会补上未闭合的代码块围栏
- **`/do <任务>`（别名 `/agent`）**：把任务交给 `dsh --profile headless` 执行，结果写回本会话
  - 便宜的思考层（REPL）+ 昂贵的执行层（DSH agent）共享上下文：先在对话里把问题聊清楚，再把提炼后的任务连同最近对话摘要交给 agent 动手
  - 执行前必须确认，并明确告知会真实修改文件、执行命令；任务作为**单个 argv 元素**传递，shell 元字符不会被展开
  - 沿用既有信任边界：子进程环境里的 `DEEPSEEK_API_KEY` / `DEEPSEEK_BASE_URL` 仍被剥除，DSH 用它自己的凭据
  - 支持 Esc 中断与超时；输出同样走 Markdown 渲染
- **`/race [档位…] <问题>`**：同一个问题在多个思考强度上并行提问，按耗时 / 输出 token / 思考 token / 花费对比后保留一个
  - `low` / `high` / `max` 到底何时值得，此前只能靠猜；这个客户端本来就在精确计量，于是把玄学变成针对你自己那类问题的实测
  - 未选中的分支不写入会话，但**同样计费**，因此三个分支的用量都会计入会话累计（界面会说明）
- **`/cache`**：上下文缓存命中率、可复用前缀大小与花费估算
- **成本估算**：每轮页脚与 `/status`、`/cache` 显示 `≈$x`。价目按 DeepSeek 公布的 USD/1M token，取**峰值价**（低谷价约为一半），因此估算只会偏高不会偏低；可在 `config.json` 的 `pricing` 里按模型覆盖。没有内置价目的模型只显示 token，不会编造金额
- **缓存失效前置警告**：DeepSeek 的上下文缓存要求前缀**完全匹配**，因此 `/compact` 与切换模型会让整段前缀作废。两者现在都会先说明将损失多少 token、下一轮多付多少钱，再让你确认

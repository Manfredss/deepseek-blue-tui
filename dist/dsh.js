...
const DSH_MISSING_MESSAGE = "未找到 DSH。请任选一种方式安装：\n" +
...
function dshManifestVersion(manifestPath) {
    try {
        const manifest = JSON.parse(readFileSync(manifestPath, "utf8"));
        if (!isRecord(manifest) || manifest.name !== DSH_PACKAGE_NAME)
            return undefined;
 ...
 */
export function detectDshVersion(commandPath) {
    let current;
    try {
...
function manifestCommand(requireFrom) {
    try {
        const manifestPath = requireFrom.resolve(`${DSH_PACKAGE_NAME}/package.json`);
        const manifest = JSON.parse(readFileSync(manifestPath, "utf8"));
        if (!isRecord(manifest) || !isRecord(manifest.bin) || typeof manifest.bin.dsh !== "string")
            return undefined;
...
export function resolveDshCommand(options = {}) {
    ...
    if (env.DEEPSEEK_DSH_COMMAND?.trim()) {
        ...
        if (isAbsolute(command) && !executable(command))
            return undefined;
...
export function dshChildEnvironment(env = process.env) {
    const childEnv = { ...env };
...
function looksLikeDsh(pid, spawnImpl = spawnSync) {
...
function validState(value) {
    if (!isRecord(value))
        return undefined;
    if (!Number.isInteger(value.pid) || Number(value.pid) <= 1)
        return undefined;
    if (!Number.isInteger(value.port) || Number(value.port) <= 0 || Number(value.port) > 65_535)
        return undefined;
    ...
    if (typeof value.dshVersion === "string")
        state.dshVersion = value.dshVersion;
    return state;
}
 ...
 */
export async function installDsh(options = {}) {
    const env = options.env ?? process.env;
    ...
    const installResult = spawnImpl(npm, ["install", "--global", "--no-fund", "--no-audit", spec], { stdio: "inherit", env, timeout: 10 * 60_000, shell });
    if (installResult.error)
        throw new Error(`无法运行 npm：${installResult.error.message}`);
    if (installResult.status !== 0)
        throw new Error(`DSH 安装失败（npm 退出码 ${installResult.status}）`);
...
export class DshManager {
     ...
     */
    constructor(home, psImpl = spawnSync) {
     ...
     */
    resolveCommand(env = process.env) {
     ...
     */
    installedVersion(env = process.env) {
        return this.resolveCommand(env)?.version;
    }
    async readState() {
        try {
            return validState(JSON.parse(await readFile(this.statePath, "utf8")));
        }
        catch (error) {
    ...
    async status(port = 3080) {
        ...
        const warning = state.dshVersion && installed && state.dshVersion !== installed
            ? `正在运行的 DSH ${state.dshVersion} 与当前安装的 ${installed} 不一致，可用 /dsh restart 切换`
            : undefined;
        if (alive && open)
            return { phase: "running", ...common, ...(warning ? { warning } : {}) };
        if (alive)
            return { phase: "starting", ...common, ...(warning ? { warning } : {}) };
        if (open) {
    ...
    async start(options = {}) {
        ...
        if (!Number.isInteger(port) || port <= 0 || port > 65_535)
            throw new Error("DSH 端口必须在 1 到 65535 之间");
        ...
        if (!command)
            throw new Error(DSH_MISSING_MESSAGE);
        ...
        if (command.version)
            state.dshVersion = command.version;
    ...
    async runHeadless(options) {
        ...
        if (!command)
            throw new Error(DSH_MISSING_MESSAGE);
        const child = spawn(command.command, [...command.argsPrefix, "--profile", "headless", task], {
            cwd: options.cwd,
            env: dshChildEnvironment(),
        ...
        try {
            ...
            const result = { output: output.trim(), status, timedOut };
            if (errors.trim())
                result.stderr = redactSecrets(errors.trim());
            return result;
        }
        finally {
            clearTimeout(timer);
    ...
    async stop() {
        const state = await this.readState();
        if (!state)
            return { stopped: false, message: "没有由 deepseek 管理的 DSH 进程" };
        ...
        if (process.platform === "win32") {
            ...
            if (killed.error)
                throw new Error(`无法停止 Windows DSH 进程：${killed.error.message}`);
        }
        else {
...
export function formatDshStatus(status) {
    const labels = {
        running: "运行中",
        starting: "启动中",
        stopped: "已停止",
    ...
    if (status.version)
        details.push(`DSH ${status.version}`);
    if (status.warning)
        details.push(status.warning);
    return details.join(" · ");
}
export function dshExecutableName(command) {

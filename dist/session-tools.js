...
export const COMPACT_INSTRUCTION = "请把以上全部对话压缩为一段紧凑、信息完整的中文摘要，保留：所有关键结论与决策、未完成的任务与下一步计划、重要代码片段与错误信息、用户的偏好和约束。只输出摘要本身，不要任何开场白。";
...
export function buildContextBreakdown(messages, limitTokens, width = 20) {
    ...
    const reasoning = messages.reduce((sum, message) => sum + (message.reasoningContent ? estimateTextTokens(message.reasoningContent) : 0), 0);
...
export async function writeSessionExport(session, directory) {
...
export function searchMessages(messages, query, limit = 12) {
...
export function applyCompactSummary(summary) {
...
export function forkSessionAt(session, index) {
...
export function userMessageIndexes(session) {
    const indexes = [];
...
export async function readAttachmentFile(input, cwd, maxBytes = ATTACH_MAX_BYTES) {
    let expanded = input.trim();
    if (!expanded)
        return { ok: false, error: "用法：/attach <文件路径>" };
    if (expanded === "~")
        expanded = homedir();
    ...
    try {
        ...
        if (code === "EACCES" || code === "EPERM")
            return { ok: false, error: `没有读取权限：${path}` };
        if (code === "ELOOP")
            return { ok: false, error: `符号链接循环：${path}` };
        if (code === "ENAMETOOLONG")
            return { ok: false, error: `路径过长：${path}` };
        if (code === "ENOTDIR")
            return { ok: false, error: `路径中有一段不是目录：${path}` };
        throw error;
    }
    if (!info.isFile())
        return { ok: false, error: `不是普通文件：${path}` };
    ...
    if (buffer.includes(0))
        return { ok: false, error: `看起来是二进制文件，无法作为文本附加：${path}` };
    const content = buffer.toString("utf8").replace(/\r\n/g, "\n").replace(/\n{3,}$/g, "\n\n").trimEnd();
    if (!content)
        return { ok: false, error: `文件为空：${path}` };
    return { ok: true, path, content };
}
/** Builds the @-referenced message body sent for an attached file. */
export function attachmentMessage(path, content) {
    return `@${path}\n\`\`\`\n${content}\n\`\`\``;
}
...
export async function editPrompt(options) {
    ...
    if (result.status !== 0)
        return { ok: false, error: `编辑器退出码 ${result.status}，已保留原输入` };
    const text = (await readFile(draft, "utf8")).trimEnd();
    if (!text.trim())
        return { ok: false, error: "编辑器内容为空，已取消" };
    return { ok: true, text };
}

...
function isZeroWidth(character, codePoint) {
 ...
 */
export function clipToWidth(value, width) {
    if (width <= 0)
        return "";
 ...
 */
export function shortenPath(value, width, home) {
    const normalized = stripAnsi(value).replace(/[\r\n\t]/gu, " ").trim();
    if (!normalized)
        return ".";
    const root = home?.replace(/[\\/]+$/u, "");
    const collapsed = root && root.length > 1 && (normalized === root || /^[\\/]/u.test(normalized.slice(root.length)))
        ? `~${normalized.slice(root.length)}`
        : normalized;
    if (width <= 0)
        return "";
    if (visibleWidth(collapsed) <= width)
        return collapsed;
    if (width === 1)
        return "…";
...
export function padToWidth(value, width, align = "left") {
    if (width <= 0)
        return "";

...
export const SLASH_COMMAND_DEFINITIONS = [
    ...
    { command: "/cache", description: "上下文缓存命中率与花费估算" },
    ...
    { command: "/race", description: "多个思考强度并行对比，保留一个" },
    ...
    { command: "/do", description: "把任务交给 DSH 执行（会修改文件）" },
...
export function unescapePrompt(input) {
...
export function slashCommandSuggestions(line, limit = SLASH_COMMAND_DEFINITIONS.length) {
    if (!line.startsWith("/") || line.startsWith("//") || /\s/u.test(line))
        return [];
...
function editDistance(left, right) {
    ...
    for (let row = 1; row <= left.length; row += 1) {
        ...
        for (let column = 1; column <= right.length; column += 1) {
            const candidate = Math.min((previous[column] ?? 0) + 1, (previous[column - 1] ?? 0) + 1, diagonal + (left[row - 1] === right[column - 1] ? 0 : 1));
...
export function renderSlashCommandMenu(line, options) {
    ...
    const selected = requestedSelected === undefined || !Number.isFinite(requestedSelected)
        ? -1
    ...
    for (let index = 0; index < visible.length; index += 1) {
        ...
        const commandText = () => options.theme.brightBlue(command);
        const highlightedCommand = () => options.theme.bold(options.theme.brightBlue(command));
        if (columns < 44) {
        ...
        rendered.push(`${isSelected ? options.theme.brightBlue("❯ ") : "  "}${isSelected ? highlightedCommand() : commandText()}${commandPadding} ${options.theme.muted(clipToWidth(description, descriptionWidth))}`);
    }
    if (hiddenBefore > 0 || hiddenAfter > 0) {
        const overflow = hiddenBefore > 0 && hiddenAfter > 0
...
const COMMAND_HELP_ROWS = [
    ...
    ["/cache", "缓存命中率、可复用前缀与花费估算"],
    ["/btw <问题>", "侧问：复用上下文单轮问答，不写入会话"],
    ["/race [档位…] <问题>", "同一问题多档并行，按耗时/token/花费对比后保留一个"],
    ...
    ["/do <任务>", "把任务交给 DSH 智能体执行，结果写回本会话（别名 /agent）"],
...
function helpSection(theme, title, rows) {
    const accent = theme?.brightBlue ?? ((value) => value);
    const bold = theme?.bold ?? ((value) => value);
    const muted = theme?.muted ?? ((value) => value);
...
export function commandHelp(theme) {
    const muted = theme?.muted ?? ((value) => value);
    return [

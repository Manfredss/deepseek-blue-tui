 ...
 */
function tintCode(line, theme, state) {
    if (!theme.enabled)
        return line;
    let out = "";
    let index = 0;
    const flushWord = (word) => {
        if (!word)
            return "";
        if (/^\d[\d_.eE+-]*$/u.test(word) || /^0[xXbBoO][0-9a-fA-F_]+$/u.test(word))
            return theme.cyan(word);
...
function inlineMarkdown(text, theme) {
    if (!theme.enabled)
        return text;
    let result = "";
    let index = 0;

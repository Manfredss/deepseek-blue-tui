...
async function fetchWithAbort(fetchImpl, url, init, signal) {
    ...
    return await new Promise((resolveResponse, rejectResponse) => {
        ...
        const abort = () => finish(() => rejectResponse(signal.reason ?? new DOMException("Aborted", "AbortError")));
        signal.addEventListener("abort", abort, { once: true });
        void fetchImpl(url, { ...init, signal }).then((response) => finish(() => resolveResponse(response)), (error) => finish(() => rejectResponse(error)));
    });
}
/**
...
async function errorFromResponse(response) {
    ...
    try {
        ...
        if (isRecord(value) && isRecord(value.error)) {
            if (typeof value.error.message === "string")
                message = value.error.message.trim();
    ...
    if (!message)
        message = hint ?? `DeepSeek API 请求失败 (${response.status})`;
    else if (hint)
        message = `${message}（${hint}）`;
    return new DeepSeekApiError(message, response.status, code);
}
function linkedAbortController(signal, timeoutMs, timeoutMessage) {
...
export async function streamChat(options) {
    ...
    const { controller, dispose } = linkedAbortController(options.signal, options.timeoutMs ?? 10 * 60_000, `DeepSeek API 请求超时：${options.baseUrl}`);
    try {
        ...
        if (!response.body)
            throw new DeepSeekApiError("DeepSeek API 未返回响应流", response.status);
        let content = "";
        ...
        const result = { content, reasoningContent, usage };
        ...
        if (controller.signal.aborted && !(error instanceof DeepSeekApiError)) {
            ...
            if (reason instanceof DeepSeekTimeoutError)
                throw reason;
...
export async function getBalance(options) {
    const { controller, dispose } = linkedAbortController(options.signal, options.timeoutMs ?? 15_000, `余额查询超时：${options.baseUrl}`);
    try {
        ...
        const balances = value.balance_infos.flatMap((item) => {
            if (!isRecord(item))
                return [];

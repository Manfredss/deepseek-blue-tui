...
import { SessionStore, addUsage, createSession, deriveTitle, planRequest, } from "./session-store.js";
...
async function requestedSession(store, cwd, model, request, continueLast) {
    if (typeof request === "string") {
        const matches = await store.find(request, cwd);
        if (matches.length === 0)
            throw new Error(`未找到会话：${request}`);
        if (matches.length > 1)
            throw new Error(`会话匹配不唯一：${request}`);
...
export async function runOneShot(options) {
    ...
    if (!runtime.apiKey)
        throw new Error("缺少 API Key。请运行 deepseek login 或设置 DEEPSEEK_API_KEY");
    const session = await requestedSession(options.sessionStore, options.cwd, options.config.model, options.resume, options.continueLast ?? false);
    ...
    try {
        ...
        const user = { role: "user", content: options.prompt, createdAt: new Date().toISOString() };
        session.messages.push(user);
        if (session.title === "New conversation")
            session.title = deriveTitle(options.prompt);
 ...
 */
function truncateForSend(session, limitTokens, stderr) {
    const plan = planRequest(session.messages, limitTokens);
    if (plan.dropped > 0) {
        stderr.write(`警告：上下文超限（估算 ${plan.estimated.toLocaleString()} tokens > ${limitTokens.toLocaleString()}），本次请求省略最早的 ${plan.dropped} 条消息（本地历史保留）。\n`);
    }
    else if (plan.nearLimit) {
        stderr.write(`警告：上下文较长（估算 ${plan.estimated.toLocaleString()} tokens，上限 ${limitTokens.toLocaleString()}）。\n`);
    }
    return plan.messages;
}
/** Reads a piped prompt so `cat notes.md | deepseek` works in scripts. */
async function readPipedPrompt(stream = process.stdin) {
    const chunks = [];
    for await (const chunk of stream)
        chunks.push(Buffer.from(chunk));
    return Buffer.concat(chunks).toString("utf8").trim();
}
async function standaloneLogin(store, config) {
...
async function standaloneUsage(store, config, args) {
...
async function listSessions(store, cwd) {
...
function parseDshOptions(args, defaultPort) {
    let action = "start";
    ...
    try {
        ...
        if (options.baseUrl) {
            ...
            if (!isValidBaseUrl(baseUrl))
                throw new Error("--endpoint 必须是有效的 http(s) URL");
            config.baseUrl = baseUrl;
        }
        if (options.showReasoning)
            config.showReasoning = true;
        if (options.effort) {
            if (!isValidEffort(options.effort))
                throw new Error("--effort 必须是 low、high 或 max");
            config.effort = options.effort;
        }
        if (options.command === "login")
            await standaloneLogin(configStore, config);
        else if (options.command === "usage")
            await standaloneUsage(configStore, config, options.commandArgs);
        else if (options.command === "sessions")
            await listSessions(sessionStore, cwd);
        else if (options.command === "dsh")
            await runDsh(dshManager, config, options.commandArgs, cwd);

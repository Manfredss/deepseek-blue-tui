...
import { addUsage, createSession, deriveTitle, estimateTextTokens, estimateTokens, planRequest, } from "./session-store.js";
import { applyCompactSummary, attachmentMessage, buildContextBreakdown, COMPACT_INSTRUCTION, editPrompt, forkSessionAt, formatCompactTokens, readAttachmentFile, searchMessages, userMessageIndexes, writeSessionExport, } from "./session-tools.js";
import { ensurePrivateDirectory } from "./fs-utils.js";
import { closestCommands, commandHelp, parseSlashCommand, renderSlashCommandMenu, slashCommandSuggestions, unescapePrompt, } from "./commands.js";
import { colorEnabled, createTheme } from "./theme.js";
import { renderWelcomeScreen } from "./logo.js";
import { LineInput, promptSecret, MenuPicker, watchAbortKeys } from "./input.js";
import { DeepSeekApiError, getBalance, streamChat } from "./api.js";
import { DEEPSEEK_URLS, openUrl } from "./open-url.js";
import { formatDshStatus, installDsh } from "./dsh.js";
...
import { cacheReport, estimateCost, formatCost, prefixLoss, pricingFor } from "./pricing.js";
...
const DSH_NOT_FOUND_HINT = "未找到 DSH，无法执行任务。输入 /dsh install 安装，或用 DEEPSEEK_DSH_COMMAND 指向已有的 dsh。";
function safeTerminalText(value) {
    return value.replace(/[\u0000-\u0008\u000b-\u001f\u007f-\u009f]/g, "");
}
function shortText(value, width = 900) {
...
export class DeepSeekTui {
    ...
    renderHome() {
        ...
        if (this.showLogo) {
            if (this.theme.enabled)
                this.write("\u001b[0m");
            this.write(renderWelcomeScreen(this.theme, {
        ...
        if (this.session.messages.length > 0)
            this.renderHistory(this.session);
        const columns = this.terminalColumns();
        this.line(this.theme.muted("─".repeat(columns)));
        this.line(clipToWidth(`${this.theme.blue("●")} ${this.theme.bold(this.session.model)} ${this.theme.muted(`· 思考强度 ${this.config.effort}`)} ${this.theme.muted(columns < 40 ? "· /help" : "· /help 查看命令")}`, columns));
        if (columns >= 72) {
        ...
        if (!runtime.apiKey) {
            this.line(this.theme.yellow(columns < 48 ? "未配置 API Key · /login" : "尚未配置 API Key。输入 /login 可安全粘贴，或打开 DeepSeek 平台。"));
        }
        this.line();
    }
    /** Clears the viewport; `scrollback` also drops the terminal's history. */
    clearScreen(scrollback = false) {
        if (!this.output.isTTY)
            return;
        this.write(scrollback ? "\u001b[2J\u001b[3J\u001b[H" : "\u001b[2J\u001b[H");
    }
    redrawHomeForResize() {
        if (!this.mainPromptActive || !this.homeScreenPristine || !this.session || this.controller)
            return;
        this.clearScreen();
        this.renderHome();
    }
    ...
    async initialSession() {
        ...
        if (typeof this.resumeQuery === "string") {
            ...
            if (matches.length === 1 && matches[0])
                return matches[0];
            if (matches.length > 1)
                return await this.chooseSession(matches) ?? createSession(this.cwd, this.config.model);
    ...
    async lockSession(session) {
        ...
        try {
            ...
            this.line(this.theme.yellow(pid !== undefined
    ...
    async run() {
        if (!this.inputStream.isTTY || !this.output.isTTY) {
        ...
        try {
            ...
            while (!this.exitRequested) {
                ...
                try {
                    if (command)
                        await this.handleCommand(command.name, command.args, command.tokens);
                    else
                        await this.sendMessage(unescapePrompt(line));
                }
                catch (error) {
                    // A failing command (unreadable file, full disk, offline DSH) must
                    // never take the whole REPL down with it.
                    if (error.name === "AbortError")
                        this.line(this.theme.yellow("已取消。"));
                    else
                        this.line(this.theme.red(`出错了：${this.errorMessage(error)}`));
                }
                finally {
    ...
    beginGenerationGuard() {
        ...
        return {
            detach: () => {
                ...
                this.input.resumeFromMenu();
                if (leftover)
                    this.input.pushText(leftover);
            },
        };
    }
    async chooseSession(sessions) {
        ...
        const result = await this.runMenu({
            title: "最近会话",
            items: visible.map((session, index) => `${String(index + 1).padStart(2)}  ${formatTime(session.updatedAt)}  ${oneLine(session.title, 200)}`),
            footer: "↑/↓ 选择 · Enter 确认 · Esc 取消 · 数字跳转",
        });
        if (!result || result.kind !== "index")
            return undefined;
        return visible[result.index];
    }
    async handleCommand(name, args, tokens) {
        switch (name) {
            case "help":
    ...
    async changeModel(requested) {
        ...
        if (!model) {
            const result = await this.runMenu({
                title: "选择模型",
                items: RECOMMENDED_MODELS.map((item, index) => `${index + 1}  ${item.label}${this.session?.model === item.id ? " ✓" : ""}  ${item.description}`),
                allowCustom: true,
                customLabel: "自定义模型：",
        ...
        if (this.session && model !== this.session.model && !(await this.confirmPrefixLoss("切换模型")))
            return;
    ...
    async logout() {
        ...
        if (environmentStillSet)
            this.line(this.theme.yellow("DEEPSEEK_API_KEY 环境变量仍然生效。"));
    }
    async usage(mode) {
        const runtime = this.configStore.runtime(this.config);
        if (runtime.apiKey) {
            try {
                const balance = await this.withSpinner("正在查询余额", () => getBalance({ apiKey: runtime.apiKey, baseUrl: runtime.baseUrl }));
                if (balance.balances.length === 0)
                    this.line(this.theme.muted("余额接口未返回币种明细。"));
                for (const item of balance.balances) {
                    this.line(`${this.theme.bold(item.totalBalance)} ${item.currency} ${this.theme.muted(`(充值 ${item.toppedUpBalance} · 赠金 ${item.grantedBalance})`)}`);
                }
                if (!balance.available)
                    this.line(this.theme.yellow("当前余额不可用于 API 调用。"));
            }
            catch (error) {
    ...
    async resumeConversation(query) {
        ...
        if (matches.length === 1)
            ...
            selected = await this.chooseSession(matches);
        if (!selected) {
            if (query.trim() && matches.length === 0)
                this.line(this.theme.yellow(`未找到会话：${query.trim()}`));
            return;
        }
    ...
    async changeEffort(requested) {
        ...
        if (normalized && REASONING_EFFORTS.includes(normalized)) {
            ...
            const result = await this.runMenu({
                title: "选择思考强度",
                items: REASONING_EFFORTS.map((level) => `${level.padEnd(4)} ${REASONING_EFFORT_LABELS[level]}${this.config.effort === level ? " ✓" : ""}`),
                footer: "↑/↓ 选择 · Enter 确认 · Esc 取消 · 数字跳转",
            });
            if (!result || result.kind !== "index")
                return;
        ...
        await this.configStore.save(this.config);
        this.line(`${this.theme.green("✓")} 思考强度已设为 ${this.theme.bold(effort)} ${this.theme.muted(`· ${REASONING_EFFORT_LABELS[effort]}`)}`);
    }
    ...
    renderHud() {
        if (!this.session)
            return "";
        const runtime = this.configStore.runtime(this.config);
        return renderContextHud(this.theme, {
            model: this.session.model,
    ...
    renderTpsLine() {
        ...
        if (tps >= 50)
            return this.theme.green(value);
        if (tps >= 20)
            return this.theme.yellow(value);
    ...
    async showContext() {
        ...
        const runtime = this.configStore.runtime(this.config);
        this.line(renderContextReport(this.theme, {
            model: this.session.model,
        ...
        if (messages.length > head.length)
            this.line(this.theme.muted(`… 已省略更早的 ${messages.length - head.length} 条`));
        const labels = { system: "系统", user: "用户", assistant: "助手" };
        head.forEach((message, index) => {
            ...
            this.line(clipToWidth(`  ${this.theme.blue(String(offset).padStart(3))}  ${this.theme.muted(labels[message.role])} ${this.theme.muted(tokens.padStart(6))}  ${oneLine(message.content, 200)}`, this.terminalColumns()));
        });
        ...
        for (const segment of breakdown.segments) {
            ...
            this.line(`  ${this.theme.muted(label.padEnd(4))} ${this.theme.blue("█".repeat(Math.max(1, Math.round(segment.percent / 100 * 20))).padEnd(20, " "))} ${formatCompactTokens(segment.tokens).padStart(7)} (${segment.percent.toFixed(0)}%)`);
        }
    ...
    async sideQuestion(prompt) {
        ...
        const spinner = new Spinner(this.output, (frame, elapsedMs) => this.generationStatus(frame, elapsedMs, reasoning, "侧问中"));
        spinner.start();
        try {
            ...
            if (!contentShown && !reasoningShown)
                this.write(this.theme.muted("(没有文本响应)"));
     ...
     */
    async raceEfforts(args) {
        if (!this.session)
            return;
        const tokens = args.trim().split(/\s+/u).filter(Boolean).map((value) => value.toLocaleLowerCase());
        const efforts = tokens.filter((token) => REASONING_EFFORTS.includes(token));
        const prompt = tokens.filter((token) => !REASONING_EFFORTS.includes(token)).join(" ");
        ...
        const generationGuard = this.beginGenerationGuard();
        const done = new Set();
        const spinner = new Spinner(this.output, (frame, elapsedMs) => clipToWidth(`${this.theme.blue(frame)} ${this.theme.muted(`并行生成中… (${String(Math.max(0, Math.round(elapsedMs / 1_000)))}s · 已完成 ${done.size}/${levels.length} · esc 中断)`)}`, this.terminalColumns()));
        this.write("\n");
        spinner.start();
        const pricing = this.pricing();
        try {
            const results = await Promise.all(levels.map(async (effort) => {
            ...
            this.line(this.theme.muted(`  ${padToWidth("档位", 6)}${padToWidth("耗时", 9)}${padToWidth("输出", 8)}${padToWidth("思考", 8)}${pricing ? padToWidth("花费", 10) : ""}摘要`));
            for (const entry of results) {
                ...
                this.line(clipToWidth(`  ${this.theme.bold(padToWidth(entry.effort, 6))}${padToWidth(`${(entry.elapsedMs / 1_000).toFixed(1)}s`, 9)}${padToWidth(formatCompactTokens(entry.usage.completionTokens), 8)}${padToWidth(formatCompactTokens(entry.usage.reasoningTokens), 8)}${cost}${this.theme.muted(oneLine(entry.content, 200))}`, columns));
            }
            ...
            const choice = await this.runMenu({
                ...
                items: usable.map((entry) => `${entry.effort.padEnd(5)} ${(entry.elapsedMs / 1_000).toFixed(1)}s  ${oneLine(entry.content, 160)}`),
            ...
            const assistant = { role: "assistant", content: kept.content, createdAt: new Date().toISOString() };
            if (kept.reasoning)
                assistant.reasoningContent = kept.reasoning;
            this.session.messages.push(assistant);
            // Every branch was billed, so charge the session for all of them.
            for (const entry of results)
                this.session.usage = addUsage(this.session.usage, entry.usage);
            ...
            if (this.session.title === "New conversation")
                this.session.title = deriveTitle(question);
            await this.saveSession();
    ...
    async compactConversation() {
        ...
        const estimated = estimateTokens(this.session.messages);
        this.line(`将把 ${substantive} 条消息压缩为一段摘要并替换会话历史（估算 ${formatCompactTokens(estimated)} tokens → 摘要）。`);
        const runtime = this.configStore.runtime(this.config);
        if (!runtime.apiKey) {
            this.line(this.theme.yellow("缺少 API Key。请先输入 /login，或设置 DEEPSEEK_API_KEY。"));
            return;
        }
        if (!(await this.confirmPrefixLoss("压缩")))
            return;
        ...
        this.controller = new AbortController();
        const generationGuard = this.beginGenerationGuard();
        const spinner = new Spinner(this.output, (frame, elapsedMs) => this.generationStatus(frame, elapsedMs, "", "正在压缩历史"));
        this.write("\n");
        spinner.start();
        const startedAt = Date.now();
        try {
            ...
            this.line(`${this.theme.green("✓")} 已压缩为 1 条摘要消息：${formatCompactTokens(estimated)} → ${formatCompactTokens(after)} tokens`);
    ...
    async rewindConversation(token) {
        if (!this.session)
            return;
    ...
    async handleDsh(tokens) {
        ...
        try {
            ...
            const status = await this.withSpinner("正在启动/连接 DSH Web", () => this.dsh.start({ port, cwd: this.cwd }));
            this.line(formatDshStatus(status));
            if (status.phase === "running")
                await this.openAndReport(status.url, "DSH Web");
            else
                this.line(this.theme.muted(`仍在启动；日志位于 ${status.logFile ?? this.dsh.logPath}`));
        }
        catch (error) {
     ...
     */
    async withSpinner(label, task) {
        const spinner = new Spinner(this.output, (frame, elapsedMs) => clipToWidth(`${this.theme.blue(frame)} ${this.theme.muted(`${label}… (${String(Math.max(0, Math.round(elapsedMs / 1_000)))}s)`)}`, this.terminalColumns()));
        spinner.start();
        try {
            return await task();
        }
    ...
    async delegateToAgent(task) {
        ...
        if (context)
            this.line(`  ${this.theme.muted(padToWidth("上下文", 6))}  ${this.theme.muted(`附带最近 ${formatCompactTokens(estimateTextTokens(context))} tokens 的对话摘要`)}`);
        ...
        const generationGuard = this.beginGenerationGuard();
        let bytes = 0;
        const spinner = new Spinner(this.output, (frame, elapsedMs) => clipToWidth(`${this.theme.blue(frame)} ${this.theme.muted(`DSH 执行中… (${String(Math.max(0, Math.round(elapsedMs / 1_000)))}s · ${formatCompactTokens(bytes / 4)} 输出 · esc 中断)`)}`, this.terminalColumns()));
        this.write("\n");
        spinner.start();
        const startedAt = Date.now();
        try {
            ...
            if (!result.output) {
                ...
                if (result.stderr)
                    this.line(this.theme.muted(safeTerminalText(result.stderr).slice(0, 2_000)));
                return;
            }
            ...
            if (error.name === "AbortError")
                ...
                this.line(`${this.theme.red(`DSH 执行失败：${this.errorMessage(error)}`)}\n`);
        }
        finally {
            spinner.stop();
            generationGuard.detach();
    ...
    requestMessages(messages) {
        ...
        if (plan.dropped > 0) {
            this.line(this.theme.yellow(`上下文超限（估算 ${plan.estimated.toLocaleString()} > ${limit.toLocaleString()} tokens），本次请求省略最早的 ${plan.dropped} 条消息。`));
            this.line(this.theme.muted("本地历史仍然完整；可用 /compact 压缩，或在 config.json 调高 contextLimitTokens。"));
        }
        else if (plan.nearLimit) {
            this.line(this.theme.yellow(`上下文较长（估算 ${plan.estimated.toLocaleString()} tokens，上限 ${limit.toLocaleString()}）。可用 /compact 压缩历史。`));
        }
        return plan.messages;
    }
    /** Live status line shown while a request is in flight. */
    generationStatus(frame, elapsedMs, reasoning, label) {
        const parts = [`${String(Math.max(0, Math.round(elapsedMs / 1_000)))}s`];
        if (reasoning.length > 0)
            parts.push(`思考 ${formatCompactTokens(estimateTextTokens(reasoning))} tokens`);
        parts.push("esc 中断");
        return clipToWidth(`${this.theme.blue(frame)} ${this.theme.muted(`${label}… (${parts.join(" · ")})`)}`, this.terminalColumns());
    }
    /**
    ...
    turnFooter(usage, elapsedMs) {
        ...
        if (usage.reasoningTokens > 0)
            parts.push(`${usage.reasoningTokens.toLocaleString()} thinking`);
        if (usage.promptCacheHitTokens > 0)
            parts.push(`缓存 ${formatCompactTokens(usage.promptCacheHitTokens)}`);
        parts.push(`${seconds.toFixed(1)}s`);
        if (usage.completionTokens > 0)
            parts.push(`${(usage.completionTokens / seconds).toFixed(1)} tok/s`);
        const pricing = this.pricing();
        if (pricing)
            parts.push(`≈${formatCost(estimateCost(usage, pricing))}`);
        return parts.join(" · ");
    }
    async sendMessage(text) {
        ...
        let contentShown = false;
        const markdown = new MarkdownStream({ theme: this.theme });
        const spinner = new Spinner(this.output, (frame, elapsedMs) => this.generationStatus(frame, elapsedMs, reasoning, "正在思考"));
        this.write("\n");
        spinner.start();
        try {
            const result = await streamChat({
            ...
            if (content.trim()) {
                const partial = { role: "assistant", content, createdAt: new Date().toISOString() };
                if (reasoning)
                    partial.reasoningContent = reasoning;
                ...
                if (this.session.messages.length === 0)
                    this.session.title = titleBefore;
                await this.saveSession();
     ...
     */
    async confirmPrefixLoss(action) {
        if (!this.session || this.session.messages.length === 0)
            return true;
        ...
        this.line(this.theme.yellow(`${action}会重写会话前缀：约 ${formatCompactTokens(loss.tokens)} tokens 的上下文缓存将失效${money}。`));
    ...
    showCache() {
        ...
        if (report.hitRate === undefined) {
            ...
            this.line(`  ${this.theme.muted(padToWidth("命中率", 8))}  ${tint(`${report.hitRate.toFixed(1)}%`)} ${this.theme.muted(`· 命中 ${formatCompactTokens(report.hitTokens)} · 未命中 ${formatCompactTokens(report.missTokens)}`)}`);
        }
        const resident = prefixLoss(this.session.messages, pricing);
        this.line(`  ${this.theme.muted(padToWidth("可复用", 8))}  ${formatCompactTokens(resident.tokens)} tokens ${this.theme.muted("（下一轮若不改动早期历史即可命中）")}`);
        if (pricing) {

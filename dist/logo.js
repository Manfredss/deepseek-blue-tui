...
const PIXEL_OUTLINE_RGB = [20, 38, 96];
const PIXEL_BELLY_RGB = [190, 225, 255];
const PIXEL_MOUTH_RGB = [255, 255, 255];
const PIXEL_RESET = "\u001b[0m";
const PIXEL_DEFAULT_BACKGROUND = "\u001b[49m";
function rgbColor(prefix, rgb) {
    return `\u001b[${prefix};2;${rgb[0]};${rgb[1]};${rgb[2]}m`;
}
function ansi256Color(prefix, color) {
    return `\u001b[${prefix};5;${String(color)}m`;
}
function pixelPalette(theme) {
    if (!theme.enabled)
        return undefined;
...
function pixelCell(palette, value) {
    if (value === undefined || value === ".")
        return undefined;
    return palette[value];
}
function renderPixelWhale(theme) {
...
function styleWhale(theme, value) {
    return value
        .split(/(\s+|●)/u)
        .map((part) => {
...
function topBorder(theme, width, title) {
    const innerWidth = Math.max(0, width - 2);
    if (innerWidth === 0)
        return theme.blue("╭╮".slice(0, width));
...
function singleRow(theme, width, cell) {
    const contentWidth = Math.max(0, width - 4);
    return `${theme.blue("│")} ${styleCell(theme, cell, contentWidth)} ${theme.blue("│")}`;
}
function twoColumnRow(theme, width, leftWidth, left, right) {
    const innerWidth = width - 2;
    const rightWidth = innerWidth - leftWidth - 1;
    const paddedCell = (cell, cellWidth) => {
        if (cellWidth < 2)
            return styleCell(theme, cell, cellWidth);
 ...
 */
function compactPath(value, width, home) {
 ...
 */
export function renderLogo(theme, options = {}) {
...
export function renderWelcomeScreen(theme, options) {
    ...
    if (compactHeight) {
        const compactRows = [
            ...
            ...MINI_WHALE.map((text) => ({ text, align: "center", tone: "whale" })),
        ...
        for (const row of compactRows)
            lines.push(singleRow(theme, width, row));
    }
    else if (width >= 84) {
        const innerWidth = width - 2;
        const leftWidth = Math.floor((innerWidth - 1) * 0.56);
        const hasDetailedWhaleSpace = width >= 104 && options.rows !== undefined && Number.isFinite(options.rows) && options.rows >= 24;
        const pixelWhale = renderPixelWhale(theme);
        const whale = pixelWhale ?? (hasDetailedWhaleSpace ? DETAILED_WHALE : LARGE_WHALE);
        const whaleTone = pixelWhale ? "raw" : "whale";
        const left = [
            { text: `欢迎回来，${username}`, align: "center", tone: "normal" },
            ...whale.map((text) => ({ text, align: "center", tone: whaleTone })),
            { text: `${options.model} · ${apiStatus}`, align: "center", tone: "muted" },
        ...
        for (let index = 0; index < height; index += 1) {
            lines.push(twoColumnRow(theme, width, leftWidth, left[index] ?? { text: "" }, right[index] ?? { text: "" }));
        }
    }
    else {
        const contentWidth = Math.max(0, width - 4);
        ...
        const rows = [
            { text: `欢迎回来，${username}`, align: "center" },
            ...whale.map((text) => ({ text, align: "center", tone: "whale" })),

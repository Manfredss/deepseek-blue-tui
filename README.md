...
[特性一览](#特性一览) · [快速开始](#快速开始) · [安装](#安装) · [键盘快捷键](#键盘快捷键) · [斜杠命令](#斜杠命令) · [命令行用法](#命令行用法) · [DSH 后台管理](#dsh-后台管理) · [交给 DSH 执行](#把任务交给-dsh) · [强度与花费对比](#对比思考强度与花费) · [上下文缓存](#上下文缓存) · [数据与配置](#数据配置与密钥位置) · [安全说明](#安全说明) · [当前限制](#当前限制) · [故障排查](#故障排查) · [开发与测试](#开发与测试)
...
| 📝 **Markdown 渲染** | 边流边渲染：代码块带语言标签、竖线与轻量着色，标题/列表/引用/行内代码按结构呈现；代码绝不截断或重排 |
...
| 🤖 **交给 agent 执行** | `/do <任务>` 把提炼后的任务连同对话摘要交给 `dsh --profile headless` 真正动手，结果写回会话——便宜的思考层 + 昂贵的执行层 |
| 🏁 **强度实测** | `/race low max <问题>` 同题多档并行，按耗时/token/花费对比后保留一个；把「max 值不值」从猜变成测 |
| 💰 **缓存与成本** | `/cache` 显示命中率与花费估算；`/compact`、切换模型会作废整段前缀缓存，事前告知将多付多少 |
...
| `/race [档位…] <问题>` | 同一问题在多个思考强度上并行提问，按耗时/token/花费对比后保留一个 |
...
| `/do <任务>` | 把任务交给 DSH 智能体执行，结果写回本会话（别名 `/agent`），见 [把任务交给 DSH](#把任务交给-dsh) |
...
`/dsh` 打开的是浏览器里一个全新的、没有任何上下文的会话。`/do` 不一样：它把当前对话提炼后交给 `dsh --profile headless`，让 agent 真正动手，再把结果写回本会话。

```
❯ 这个竞态我觉得在 touchLock 的 inode 比较上
◆ DeepSeek  （便宜、快速地把思路聊清楚…）

❯ /do 按上面的结论修掉，并加一个回归测试
交给 DSH 执行
  任务    按上面的结论修掉，并加一个回归测试
  目录    ~/Documents/Coding/Deepseek TUI
  DSH     /opt/homebrew/bin/dsh (v0.1.1-rc.2)
  上下文  附带最近 1.2k tokens 的对话摘要
DSH 会在上述目录里真实修改文件、执行命令，且使用它自己的凭据。
确认执行？[y/N] ›
```

分工是**便宜的思考层 + 昂贵的执行层，共享上下文**：在 REPL 里探索，把提炼出的任务交给 agent 执行。

几点约束：

- 执行前必须确认。DSH 会真实修改文件、运行命令
- 任务作为**单个 argv 元素**传给 DSH，shell 元字符不会被展开
- 沿用既有信任边界：子进程环境里的 `DEEPSEEK_API_KEY` / `DEEPSEEK_BASE_URL` 仍被剥除，DSH 使用它自己的凭据
- <kbd>Esc</kbd> 可中断；默认 15 分钟超时
- 结果以 `[DSH 执行结果]` 前缀写入会话，来源清晰可辨

## 对比思考强度与花费

`low` / `high` / `max` 何时值得，官方没有给出指引。`/race` 把它变成实测：

```
❯ /race low max 这段应该用什么数据结构缓存
并行对比 low / max
对比
  档位  耗时     输出    思考    花费      摘要
  low   1.2s     120     0       ≈$0.0004  直接用 Map 缓存即可。
  max   18.7s    1.9k    5.2k    ≈$0.0093  需要区分三种情况：…
查看并保留哪一个？（其余丢弃，不写入会话）
```

未选中的分支不写入会话，但**同样计费**，所以所有分支的用量都计入会话累计。

花费按 DeepSeek 公布的 USD/1M token 估算，取**峰值价**（低谷价约为一半），因此只会偏高不会偏低。想改用自己的价目，在 `config.json` 里按模型覆盖：
...
DeepSeek 的上下文缓存自动开启，且要求前缀**完全匹配**才命中——这意味着任何改写早期历史的操作都会让整段前缀作废。`/cache` 显示当前状况：

```
上下文缓存
  命中率    91.7% · 命中 33k · 未命中 3.0k
  可复用    12k tokens （下一轮若不改动早期历史即可命中）
  已省下    ≈$0.014 （相对全部按未命中计价）
  本会话    ≈$0.0053 · 估算，按峰值价，可在 config.json 的 pricing 覆盖
```

`/compact` 与切换模型都会作废整段前缀，两者会先告诉你将损失多少、下一轮多付多少，再让你确认。`/rewind` 只截断尾部、保留前缀，因此通常仍能命中。
